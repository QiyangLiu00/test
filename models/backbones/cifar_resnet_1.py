...
def _resnet(arch, block, layers, pretrained, progress, device, **kwargs):
    ...
    if pretrained:
        print('load ' + arch + ' pre_train weights')
        model = torch.nn.DataParallel(model).cuda()
        print(model)
        script_dir = os.path.dirname(__file__)
        state_dict = torch.load(script_dir + '/state_dicts/' + arch + '.pth')
        print(state_dict.keys())
        model.load_state_dict(state_dict, strict=False)

        if set(state_dict.keys()) != set(model.state_dict().keys()):
            raise ValueError("The state dict keys do not match the model parameters.")
    return model
...
def resnet50(pretrained=True, progress=True, device='cpu', **kwargs):
...
if __name__ == "__main__":
    ...
    script_dir = os.path.dirname(__file__)
    weights = torch.load(script_dir + '/state_dicts/resnet18.pth')
    pretrained_dict1 = {k: v for k, v in weights.items() if k not in model_dict}
    pretrained_dict2 = {k: v for k, v in weights.items() if v.shape != model_dict[k].shape}
